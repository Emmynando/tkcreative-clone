export const IMG_FILE = [
  {
    id: "tk0",
    img: "/image/tk_1.avif",
    containerStyle: {
      width: "100%",
      maxWidth: "calc(3.968rem + 39.903dvw)",
      marginLeft: "auto",
      display: "flex",
      justifyContent: "flex-end",
      marginBottom: "calc(-1.0815rem + 7.512dvw)",
    },
    imgStyle: {
      aspectRatio: 1717 / 2136,
      width: "100%",
      height: "calc(-1.778rem + -2.404dvw + 100dvh)",
    },
  },
  {
    id: "tk1",
    img: "/image/tk_2.avif",
    imgStyle: {
      aspectRatio: 1024 / 1536,
      width: "calc(40.509dvw)",
    },
  },
  {
    id: "tk2",
    img: "/image/tk_3.avif",
    imgStyle: {
      aspectRatio: 960 / 1440,
      width: "calc(23.148dvw)",
    },
  },
  {
    id: "tk3",
    img: "/image/tk_4.avif",
    imgStyle: {
      aspectRatio: 1024 / 1280,
      width: "calc(40.509dvw)",
    },
  },
  {
    id: "tk4",
    img: "/image/tk_5.avif",
    imgStyle: {
      aspectRatio: 2578 / 1719,
      width: "calc(61.921dvw)",
    },
  },
  {
    id: "tk5",
    img: "/image/tk_6.avif",
    imgStyle: {
      aspectRatio: 1835 / 2336,
      width: "calc(44.271dvw)",
    },
  },
  {
    id: "tk6",
    img: "/image/tk_7.avif",
    imgStyle: {
      aspectRatio: 1407 / 1734,
      width: "calc(33.854dvw)",
    },
  },
  {
    id: "tk7",
    img: "/image/tk_8.avif",
    imgStyle: {
      aspectRatio: 2295 / 1531,
      width: "calc(55.266dvw)",
    },
  },
  {
    id: "tk8",
    img: "/image/tk_9.avif",
    imgStyle: {
      aspectRatio: 1818 / 2575,
      width: "calc(43.692dvw)",
    },
  },
  {
    id: "tk9",
    img: "/image/tk_10.avif",
    imgStyle: {
      aspectRatio: 1289 / 1951,
      width: "calc(31.829dvw)",
    },
  },
];

export const MOBILE_IMG_FILE: {
  id: string;
  img: string;
  containerStyle?: React.CSSProperties;
  imgStyle?: React.CSSProperties;
}[] = [
  {
    id: "tk0",
    img: "/image/tk_1.avif",
    containerStyle: {
      width: "100%",
      maxWidth: "calc(-1.782rem + 97.33dvw)",
      marginBottom: "calc(4.273rem + 4.4485dvw)",
      // display: "flex",
      // justifyContent: "flex-start",
      objectFit: "cover",
      // alignItems: "center",
    },
    imgStyle: {
      aspectRatio: 1717 / 2136,
      width: "100%",
      height: "calc(-11.6222rem + 100svh - 10.678dvw)",
    },
  },
  {
    id: "tk1",
    img: "/image/tk_2.avif",
    imgStyle: {
      aspectRatio: 1024 / 1536,
      width: "calc(65.116dvw)",
    },
  },
  {
    id: "tk2",
    img: "/image/tk_3.avif",

    imgStyle: {
      aspectRatio: 960 / 1440,
      width: "calc(60.465dvw)",
    },
  },
  {
    id: "tk3",
    img: "/image/tk_4.avif",
    imgStyle: {
      aspectRatio: 1024 / 1280,
      width: "calc(65.116dvw)",
    },
  },
  {
    id: "tk4",
    img: "/image/tk_5.avif",
    imgStyle: {
      aspectRatio: 2578 / 1719,
      width: "calc(82.558dvw)",
    },
  },
  {
    id: "tk5",
    img: "/image/tk_6.avif",
    imgStyle: {
      aspectRatio: 1835 / 2336,
      width: "calc(67.442dvw)",
    },
  },
  {
    id: "tk6",
    img: "/image/tk_7.avif",
    imgStyle: {
      aspectRatio: 1407 / 1734,
      width: "calc(52.326dvw)",
    },
  },
  {
    id: "tk7",
    img: "/image/tk_8.avif",
    imgStyle: {
      aspectRatio: 2295 / 1531,
      width: "calc(82.558dvw)",
    },
  },
  {
    id: "tk8",
    img: "/image/tk_9.avif",
    imgStyle: {
      aspectRatio: 1818 / 2575,
      width: "calc(58.14dvw)",
    },
  },
  {
    id: "tk9",
    img: "/image/tk_10.avif",
    imgStyle: {
      aspectRatio: 1289 / 1951,
      width: "calc(44.186dvw)",
    },
  },
];
